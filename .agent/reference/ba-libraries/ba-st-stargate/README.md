...
- answers `412 Precondition Failed` when `If-Match` does not match on PUT/PATCH/DELETE.
...
- **`beAvailableFor:`, `doNotExpire`, `requireRevalidation`** on the builder map to Cache-Control directives — prefer them over hand-built headers.
