...
> **`relation "<table>" does not exist` against a fresh DB is usually NOT a persistence bug.** If `prepareForInitialPersistence` creates most tables but one system's table is missing — surfacing as `destroyRepositories` failing in `tearDown` — the likely cause is that **that system never started because its Kepler interface was never registered on load** (so its repository was never created and its mapping never reached the descriptor). Check the package manifest registers the interface before auditing the mapping configuration: see `abbaco-ci` (failure playbook) and `abbaco-api-domain-model` ("`registerInterfaces` must run on package load"). The warm dev image hides this; reproduce on a fresh image.
